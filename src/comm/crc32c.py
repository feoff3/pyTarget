...
def __crc32c_update(ctx, buf):
    buf = array.array('B', buf)
    for i in buf:
        ctx = (ctx >> 8) ^ __crc32c_table[(ctx ^ i) & 0xff]
    return ctx

def __crc32c_final(ctx):
    ctx = ~ctx & 0xFFFFFFFF
    buf = ctx & 0xFF, (ctx >> 8) & 0xFF, (ctx >> 16) & 0xFF, (ctx >> 24) & 0xFF
    return struct.pack('B'*4, buf[0], buf[1], buf[2], buf[3])

def crc32c(buf):
    '''
    CRC32C for header/data digest
    @param buf: string buffer
    @return: crc value
    '''
    return __crc32c_final(__crc32c_update(0xffffffff, buf))
